...
def _simulate_heuristic_episode(
    *,
    ...
    while arrival_cursor < n_tasks or pending or running_jobs:
        ...
        task_position = int(pending[0])
        demand = np.asarray(demands[task_position], dtype=np.float64)
        feasible_mask = np.all(machine_residual - demand >= -1e-9, axis=1)
...
def evaluate_rl_policy_on_episode_ids(
    *,
    ...
    for episode_idx, episode_id in enumerate(selected_episode_ids):
        ...
        unscheduled_tasks = max(0, int(total_tasks - len(assignments)))
