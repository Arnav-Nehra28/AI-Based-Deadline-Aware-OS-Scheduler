...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate the trained RL model on the validation split saved by train_model."
    )
...
def main() -> None:
    ...
    output_json = args.output_json or (run_dir / "validation_metrics.json")

    split_payload = _load_split_payload(split_path)
    val_episode_ids = [int(value) for value in split_payload.get("val_episode_ids", [])]
    if not val_episode_ids:
        raise ValueError(f"No validation episode ids found in split file: {split_path}")
    ...
    print(f"Validation episodes: {len(val_episode_ids)}")
